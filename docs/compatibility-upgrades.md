...
规则的变化。补丁 SHA256 为
`ad35c9794e72c49111cba3824db370b74f86e2db3b4f9fa515ad7c8e9c9539e6`。
...
兼容层镜像标签固定为 `v7.2.150-c77b1369-ad35c9794e72c491`，由版本号、commit
...
- 额度解析失败按固定条件返回错误码，区分 JSON、字段类型、套餐、百分比、窗口、
  重置时间和额外额度标识问题。错误不包含原始上游字段值；Gateway 使用固定白名单
  映射并写入查询审计。既有校验约束保持不变，收到错误码不等于已经证实上游改版。
