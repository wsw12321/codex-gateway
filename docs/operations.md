...
`v7.3.15-673131f5-649273bb97bef574-codex-only`；标签记录主程序、多账号补丁和仅 Codex 的构建，校验脚本会检查它，CI 使用实际构建的完整标签执行扫描。
...
5. 重启唯一 sidecar，等待健康检查，再启动配套 Gateway 并等待 `/readyz`。
   兼容层启动不等待 Gateway，避免依赖循环；生成前必须能访问 Gateway 的
   内部分配接口。
...
Owner 可在“额度与订阅”的批量操作区搜索并勾选用户；“全选当前搜索结果”只影响
当前筛选匹配的用户，切换搜索会保留之前的选择。充值金额为每人的 CNY 金额；订阅
统一选择日、周或月档位、每周期 USD 额度和周期数（1–99，0 表示无限期）。填写
操作原因后先预览用户名单、人数和参数，再确认执行。已有同档位订阅会立即关闭
旧周期并从第 1 期重开，旧周期剩余额度不结转。

各用户独立执行并记录账务流水；个别失败不会回滚已成功的用户。充值逐笔保存入账
时的汇率快照。网络中断或服务端错误可能显示“结果待确认”，此时使用“重试未成功项”
沿用原操作 ID 核对结果，成功项不会重复提交。“刷新账务数据”仅重新读取余额与
流水，不执行充值或订阅操作，也不清除当前批次。

批次仅保留在当前页面内存中，刷新页面或退出登录后无法恢复；若发生中断，先核对
各用户的账务流水再创建新批次。取消二次验证会暂停队列，当前页面内可继续重试。
...
2. 将固定多账号补丁重放到新 commit；对 `v7.3.15` 使用
...
`v7.3.15` 仓库升级沿用现有 Go 1.26 构建镜像，不更改公共 API、数据库结构或模型
...
5. 启动唯一 sidecar 和 Gateway，逐个账号执行设备码登录或刷新。Owner 的“上游
   ...
   失败，不得影响代理请求。
6. 用两个以上账号验证新会话按近 24 小时费用和系数分配（例如 5、20 对应
   20%、80% 参考目标）、同一会话一小时粘滞、账号不可用时重新绑定，
...
### 升级到 `0009_upstream_allocation.sql`

本次必须配套更新 Gateway 与 `codex-compat`。先停流量并完成数据库加密备份和
恢复演练，按固定补丁及镜像标签构建两者；启动 Gateway 时自动应用该迁移，
新增 `upstream_accounts.allocation_weight`（默认 1、范围 `0..2147483647`）与
按账号及请求时间查询账本的索引。账号同步不会覆盖系数，新登录但尚未同步到
管理页的账号首次使用自动按系数 1、无历史费用参与分配。

兼容层先通过本地健康检查，再启动 Gateway；兼容层启动不等待 Gateway，
避免启动依赖环。等 Gateway `/readyz` 返回 200 后执行
`./scripts/smoke-sidecar.sh`，然后完成真实 OAuth 普通和 SSE Responses 冒烟。
设备登录脚本已按此顺序等待两者。公网 `/internal` 与 `/internal/*` 必须返回
404；内部分配只接受 sidecar 密钥，回调不经过出口代理。

Owner 在“上游账号”页设置系数，需要近期身份验证；配置与审计在同一事务保存。
系数 5、20 表示当前候选中的目标费用占比为 20%、80%。每次新分配选择
`目标占比 × 候选总费用 − 本账号费用` 最大的账号；并列或全无历史费用时，
按系数随机选择。费用来自账本已结算 `actual_cost_usd`，跨用户、模型汇总；
请求时间在 `[当前时间−24h, 当前时间)`，老记录缺少请求时间时用账本创建时间。
不会预占费用或计入进行中请求，并发新会话短时集中属于预期行为。

设为 0 会显示“停止接收新对话”，已有有效绑定仍继续；禁用账号则遵循已有
禁用与故障切换规则。绑定有效期仍为一小时，过期或重启后重新分配。页面目标
占比按启用账号计算参考值，实际选择按当前模型及可用候选重算；近 24 小时
统计不随历史区间筛选变化。至少保留一个支持所需模型的正系数可用账号；
全部候选系数为 0、数据库或分配回调故障时，新分配返回 503，不回退轮询。

验收覆盖 0 系数排空、单候选为 0、既有绑定继续、并发首次绑定一致、两账号
故障切换与最终账号归因。迁移后旧二进制受未知迁移保护，回退必须保持停写，
将升级前备份恢复到新的隔离数据库卷，并配套回退两个镜像；不得删除迁移记录。
