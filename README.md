...
管理台的“额度与订阅”区域允许成员查看余额、三档周期和长期账务流水，并在近期
身份验证后分别为日、周、月订阅和现金余额“禁用扣费／恢复扣费”。默认四项都允许
扣费；只能修改本人设置，Owner 查看他人时也仅能查看状态。禁用仅影响新请求的
来源选择，订阅期限照常运行、周期余额不结转；未开通的订阅也可预先禁用。恢复
不会重开周期或补发额度，充值、续期和管理员重开订阅也不会重置这些设置。已受理
请求仍按准入时记录的来源结算，后来恢复的来源不会追加给旧请求。全部禁用或没有
可用额度时，普通计费请求返回 `429 insufficient_quota`，内部零价规则不变。

Owner 在近期身份验证后可按当时充值汇率进行 CNY 充值、执行带原因的正负 USD 调整、
修改充值汇率，以及立即重开或停用任一订阅档。管理员“立即停用订阅”会关闭订阅
及当前周期，与用户禁用该来源扣费不同。这些管理员账务操作要求 UUID
`operation_id`，安全重试会返回原周期配置；修改订阅会立即关闭旧周期、补满新
...
`v7.2.150-c77b1369-51e5e4bf0a2baee2-codex-only`，固定主程序、多账号补丁及移除旧 Gemini 插件后的构建。
