...
cleanup() {
    ...
    if test "$sidecar_needs_stop" -eq 1; then
        if ! compose stop -t 10 codex-compat >/dev/null 2>&1; then
            printf '%s\n' 'oauth-login: failed to stop unverified sidecar during cleanup' >&2
    ...
    if test -n "$work_dir" && test -d "$work_dir"; then
        ...
        if ! rmdir "$work_dir"; then
            printf '%s\n' 'oauth-login: failed to remove private inventory directory' >&2
            test "$status" -ne 0 || status=1
        fi
    fi
    if ! flock -u 9; then
        printf '%s\n' 'oauth-login: failed to release operation lock' >&2
        test "$status" -ne 0 || status=1
    fi
    exit "$status"
}
...
command -v flock >/dev/null 2>&1 || {
    printf '%s\n' 'oauth-login: flock from util-linux is required' >&2
    exit 1
}
if test -e "$lock_file" || test -L "$lock_file"; then
    test -f "$lock_file" && test ! -L "$lock_file" || {
        printf '%s\n' 'oauth-login: lock path must be a regular file' >&2
        exit 1
    }
fi
exec 9>"$lock_file"
...
if ! flock -n 9; then
    printf '%s\n' 'oauth-login: another login or upgrade operation holds the lock' >&2
    exit 1
fi
work_dir=$(mktemp -d /tmp/oauth-login.XXXXXX)
...
capture_oauth_inventory() {
    ...
    if ! compose run --rm --no-deps codex-compat oauth-inventory > "$inventory_raw"; then
        printf '%s\n' 'oauth-login: failed to inventory OAuth accounts' >&2
        return 1
    fi
    if ! awk '
    ...
    ' "$inventory_raw"; then
        printf '%s\n' 'oauth-login: invalid OAuth inventory response' >&2
        return 1
    fi
...
verify_single_account_mutation() {
    ...
    ' "$inventory_before" "$inventory_after"; then
        printf '%s\n' 'oauth-login: expected exactly one added or refreshed account and no removed accounts' >&2
        return 1
    fi
}
...
if test -n "$cid" && test "$(docker inspect -f '{{.State.Running}}' "$cid")" = true; then
    printf '%s\n' 'oauth-login: sidecar is still running; refusing login' >&2
    exit 1
fi
...
printf 'Starting interactive %s login. Complete authorization in your own browser.\n' "$provider"
if ! compose run --rm --no-deps codex-compat "$@"; then
    printf '%s\n' 'oauth-login: login failed; sidecar remains stopped' >&2
    exit 1
fi

if ! compose run --rm --no-deps codex-compat verify-oauth; then
    printf '%s\n' 'oauth-login: OAuth permission validation failed; sidecar remains stopped' >&2
    exit 1
fi
...
while test "$attempt" -lt 30; do
    ...
    if test -n "$cid"; then
        ...
        test "$health" = unhealthy && {
            printf '%s\n' 'oauth-login: restarted sidecar is unhealthy' >&2
            exit 1
        }
    fi
    attempt=$((attempt + 1))
...
test "${health:-}" = healthy || {
    printf '%s\n' 'oauth-login: timed out waiting for sidecar health' >&2
    exit 1
}
...
sidecar_needs_stop=0
printf 'One %s account was added or refreshed; other accounts, permissions, health, and upstream smoke checks passed\n' "$provider"
