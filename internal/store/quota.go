...
func (s *Store) ReserveQuota(ctx context.Context, params ReserveQuotaParams) (QuotaReservation, error) {
	...
	if params.LeaseTTL > 30*time.Minute {
		return QuotaReservation{}, fmt.Errorf("%w: quota lease TTL exceeds 30 minutes", ErrInvalid)
	}
	...
	err := s.withTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx,
			...
			return mapDBError("expire concurrency leases", err)
		}
		...
		if err := tx.QueryRowContext(ctx, `
			...
			return mapDBError("read concurrency leases", err)
		}
		...
		if _, err := tx.ExecContext(ctx, `
			...
			return mapDBError("initialize rate windows", err)
		}
		...
		if _, err := tx.ExecContext(ctx, `
			...
			return mapDBError("initialize quota counters", err)
		}
		...
		if err := enforceQuota("key", "daily_tokens", params.Limits.KeyDailyTokens,
			counters["key"].used+counters["key"].reserved, params.ReservedTokens, 24*time.Hour); err != nil {
			return err
		}
		if err := enforceQuota("user", "daily_tokens", params.Limits.UserDailyTokens,
			counters["user"].used+counters["user"].reserved, params.ReservedTokens, 24*time.Hour); err != nil {
			return err
		}
		...
		if err != nil {
			return mapDBError("increment rate windows", err)
		}
		...
		if err != nil {
			return mapDBError("reserve daily quota", err)
		}
		...
		if _, err := tx.ExecContext(ctx, `
			...
			return mapDBError("insert quota reservation", err)
		}
		if _, err := tx.ExecContext(ctx, `
			...
			return mapDBError("insert concurrency lease", err)
		}
...
func validateQuotaLimits(limits QuotaLimits) error {
	values := []int64{
		...
		limits.KeyDailyTokens, limits.UserDailyTokens,
	}
...
func (s *Store) ReleaseStaleQuotaReservations(ctx context.Context, before time.Time, limit int) (int, error) {
	...
	released := 0
	for _, id := range ids {
		err := s.ReleaseQuota(ctx, id, s.now().UTC())
...
func (s *Store) DeleteQuotaStateBefore(ctx context.Context, before time.Time) error {
	return s.withTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, `
			...
			WHERE state <> 'reserved' AND settled_at < $1`, before,
		); err != nil {
