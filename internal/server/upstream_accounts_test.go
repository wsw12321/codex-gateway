...
func TestUpstreamQuotaPreservesSidecarErrorClassification(t *testing.T) {
	for _, test := range []struct {
		...
		{502, "quota_percent_fractional", 502, "upstream_quota_percent_fractional", "小数百分比"},
		{502, "quota_window_invalid", 502, "upstream_quota_window_invalid", "窗口时长"},
		{502, "quota_reset_invalid", 502, "upstream_quota_reset_invalid", "重置时间"},
		{502, "quota_limit_id_invalid", 502, "upstream_quota_limit_id_invalid", "额外额度标识"},
		{502, "quota_limit_id_duplicate", 502, "upstream_quota_limit_id_duplicate", "额度标识重复"},
		{502, "quota_limits_excessive", 502, "upstream_quota_limits_excessive", "数量超过"},
